...
const [template, style, engine, app, editor, rawLayout, rawItems] = await Promise.all([
  read('src/page.html'), read('src/style.css'), read('src/engine.js'),
  read('src/app.js'), read('src/editor.js'),
  read('data/default-layout.json'), read('data/items-26.1.json')
]);
const layout = Layout.validate(JSON.parse(rawLayout));
const catalog = JSON.parse(rawItems).map(({name, displayName}) => ({name, displayName}));
const values = {STYLE: style, ENGINE: engine, APP: app, EDITOR: editor,
  DATA: safeJSON(layout), CATALOG: safeJSON(catalog)};
