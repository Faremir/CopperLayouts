...
const prepared = new ItemCatalog(require('../data/items.json')).hydrate_template(require('../config/default-layout.json'));
const original = () => Layout.validate(structuredClone(prepared));
const chest = (id, items = []) => ({
    id,
    label: id,
    items
});
...
test('migrates the accepted layout without changing any chest assignment or contents', () => {
    ...
    assert.equal(doc.chests.length, prepared.chests.length);
...
test('legacy U/D exports migrate to T/B while keeping opaque chest identities', () => {
    let current = Layout.saveFloor(empty(), {
        id: 0,
        name: 'Legacy floor',
        walls: ['L']
    });
    current = Layout.saveChest(current, chest('opaque-original-id', [{
        name: 'Oak Log',
        itemId: ''
    }]));
    current = Layout.move(current, 'opaque-original-id', 'F0L3-T1');
    ...
    assert.deepEqual(doc.placements, current.placements);
    assert.equal(doc.chests[0].id, 'opaque-original-id');
});
test('creates empty chests, accepts over 54 items, edits and deletes without altering other chests', () => {
    ...
    doc = Layout.saveChest(doc, {
        ...chest('test-chest', contents),
        label: 'Renamed'
    });
    assert.equal(doc.chests.find(c => c.id === 'test-chest').items.length, 80);
    const target = Object.keys(doc.placements)[0],
        old = doc.placements[target];
    ...
    assert.equal(doc.chests.length, prepared.chests.length);
    assert.equal(Layout.validate(doc).chests.length, prepared.chests.length);
});
test('new chests can occupy only existing empty positions', () => {
    let doc = original(),
        target = Object.keys(doc.placements)[0];
...
test('removing a wall preserves every chest exactly once and rebuilding adds empty positions', () => {
    const before = original(),
        floor = before.floors.find(f => f.id === 0);
    const wall = floor.walls[0];
    const removed = Object.entries(before.placements).filter(([s, c]) => s.startsWith(`F0${wall}`) && c).map(([, c]) => c);
    let doc = Layout.saveFloor(before, {
        ...floor,
        name: 'Workshop',
        walls: floor.walls.filter(w => w !== wall)
    });
    assert.equal(doc.staging.length, before.staging.length + removed.length);
    assert.ok(removed.every(c => doc.staging.includes(c)));
    assert.equal(doc.chests.length, before.chests.length);
    for (const [slot, c] of Object.entries(doc.placements)) {
        assert.equal(c, before.placements[slot]);
    }

    doc = Layout.saveFloor(doc, {
        ...floor,
        name: 'Workshop'
    });
    assert.equal(Object.entries(doc.placements).filter(([s, c]) => s.startsWith(`F0${wall}`) && c === null).length, 27);
    assert.ok(removed.every(c => doc.staging.includes(c)));
...
test('all floors may be removed and recreated without losing their staged chests', () => {
    ...
    for (const f of [...doc.floors]) {
        doc = Layout.deleteFloor(doc, f.id);
    }

    assert.equal(doc.floors.length, 0);
    assert.equal(doc.staging.length, prepared.chests.length);
    ...
    assert.equal(Layout.validate(doc).staging.length, prepared.chests.length - 1);
    assert.throws(() => Layout.saveFloor(doc, {
        id: 0,
        name: 'No walls',
        walls: []
    }), /wall/);
...
test('wall-facing orientation keeps bottom chests inward and facing-left column 1', () => {
    ...
    for (const [wall, order] of Object.entries(expected)) {
        assert.deepEqual(Layout.displaySlotsOf({
            id: 'square',
            wall
        }), order.map(p => 'square-' + p));
    }
...
test('export/import preserves edits and regenerates neighbors for the current walls', () => {
    ...
    doc = Layout.saveChest(doc, chest('custom', [{
        name: 'A <custom> item',
        itemId: 'example:item'
    }]));
    ...
    for (const s of exported.topology.slots) {
        for (const neighbor of [...Object.values(s.neighbors), ...s.cornerNeighbors]) {
            assert.ok(neighbor === null || ids.has(neighbor));
        }
    }
});
test('invalid imports cannot duplicate, lose, or reference unknown chests', () => {
    const doc = original(),
        slots = Object.keys(doc.placements);
    const duplicate = structuredClone(doc);
    duplicate.placements[slots[1]] = duplicate.placements[slots[0]];
    assert.throws(() => Layout.validate(duplicate), /more than once/);
    const missing = structuredClone(doc);
    missing.placements[slots[0]] = null;
    assert.throws(() => Layout.validate(missing), /missing/);
    const unknown = structuredClone(doc);
    unknown.placements[slots[0]] = 'unknown';
...
test('each wall has independent module counts and shrinking stages exactly the removed chests', () => {
    ...
    const doc = Layout.saveFloor(before, {
        ...floor,
        walls: ['T', 'R', 'B', 'L'],
        moduleCounts: {
            T: 1,
...
test('unequal wall lengths use their actual endpoints for corner neighbors', () => {
    const doc = Layout.saveFloor(empty(), {
        id: 0,
        name: 'Unequal',
        walls: ['T', 'R', 'B', 'L'],
        moduleCounts: {
    ...
    for (const slot of slots.values()) {
        for (const neighbor of slot.cornerNeighbors) {
            assert.ok(slots.get(neighbor).cornerNeighbors.includes(slot.id));
        }
    }
});
...
test('invalid wall counts are rejected before allocating slots', () => {
    for (const count of [-1, 0, 1.5, 1001, Infinity, '3']) {
        assert.throws(() => Layout.saveFloor(empty(), {
            id: 0,
            name: 'Invalid',
            walls: ['T'],
            moduleCounts: {
...
test('clear removes floors, chests, placements and staging without mutating the previous layout', () => {
    ...
    const snapshot = structuredClone(before),
        doc = Layout.clear(before);
    for (const key of ['floors', 'squares', 'chests', 'staging']) {
        assert.deepEqual(doc[key], []);
    }
...
test('floor reordering renumbers coordinates while preserving chest identities and contents', () => {
    const before = original(),
        order = [7, 0, 1, 2, 3, 4, 5, 6];
    const reordered = Layout.reorderFloors(before, order);
    assert.deepEqual(reordered.floors.map(f => f.id), [0, 1, 2, 3, 4, 5, 6, 7]);
    assert.equal(reordered.floors[0].name, before.floors[7].name);
    assert.deepEqual(reordered.chests, before.chests);
    for (const [slot, chest_id] of Object.entries(before.placements)) {
        const destination = slot.replace(/^F(\d+)/, (_, id) => 'F' + order.indexOf(Number(id)));
        assert.equal(reordered.placements[destination], chest_id);
    }

    assert.deepEqual(Layout.validate(Layout.exportDocument(reordered)), reordered);
    assert.throws(() => Layout.reorderFloors(before, [0, 0, 1, 2, 3, 4, 5, 6]), /order/);
});
